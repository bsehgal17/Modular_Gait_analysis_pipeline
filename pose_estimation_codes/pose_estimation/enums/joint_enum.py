...
class JointEnum(Enum):
    ...
    def from_name(cls, name: str) -> "JointEnum":
        ...
        try:
            ...
            raise ValueError(
                f"Unknown joint '{name}'. Valid joints: {valid}") from exc
...
MEDIAPIPE_DERIVED: dict[StandardJoint, DerivedJoint] = {
    StandardJoint.NECK: DerivedJoint(
        source_indices=[Mediapipe.LEFT_SHOULDER.value,
...
OPENPOSE_DERIVED: dict[StandardJoint, DerivedJoint] = {
    StandardJoint.HEAD: DerivedJoint(
        source_indices=[OpenPoseBODY25.LEFT_EAR.value,
                        OpenPoseBODY25.RIGHT_EAR.value]
    ),
    StandardJoint.PELVIS: DerivedJoint(
        source_indices=[OpenPoseBODY25.LEFT_HIP.value,
                        OpenPoseBODY25.RIGHT_HIP.value]
    ),
    StandardJoint.SPINE: DerivedJoint(
        source_indices=[OpenPoseBODY25.NECK.value,
                        OpenPoseBODY25.MID_HIP.value]
    ),
